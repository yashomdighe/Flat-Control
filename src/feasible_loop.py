...
class Flat_Controller:
    ...
    def __init__(self, gain, tmax, xTrajCS, yTrajCS, track, trial) -> None:
        ...
        self.ref_pub = rospy.Publisher(name="/ref_pt", data_class=Marker, queue_size=1)
    ...
    def traj_track(self, odom):
        ...
        if self.ti > self.tmax+0.01:
            ...
            with open("trials/"+self.track+"_"+str(self.tmax)+"_trial_"+self.trial+".csv", "w+") as f:
        ...
        marker.pose.position.x = x_ref
        marker.pose.position.y = y_ref
...
if __name__ == "__main__":
    ...
    ref = pd.read_csv(f"scripts/{track}_centerline_reference_Tvec_{tmax}.csv")

    coords = ref[["x_m", "y_m"]].to_numpy()
    tvec = ref[["Tvec"]].to_numpy().reshape(coords.shape[0])
    ...
    # tvec_path = "scripts/"+track+"_"+tmax+".csv"
    # tmax = 5
    # tvec = np.linspace(0,tmax,coords.shape[0])
    # tvec = t_space * tmax / t_space[-1]
    # tvec = np.genfromtxt(tvec_path, dtype=float, delimiter=",")
    # print(tvec)
    ...
    for itr,pt in enumerate(zip(xTraj, yTraj)):
        ...
        marker.type = marker.SPHERE
        marker.action = marker.ADD
        marker.scale.x, marker.scale.y, marker.scale.z = 0.25, 0.25, 0.25
        marker.color.a = 1.0
    ...
    # gain = [1,1]
    # gain = [50, 15]
    controller = Flat_Controller(gain, tmax, xTrajCS, yTrajCS, track, trial)
    r = rospy.Rate(100)
